...
extern BOOL connectA = FALSE;
unsigned __stdcall NetTickThread(LPVOID lp);
long BindRPC ();
...
unsigned __stdcall NetSetupThread(LPVOID lp)
{
 ...
 //   CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE)NetTickThread, 0, 0, &id);
	HANDLE hThread;
	unsigned threadID;
	hThread = (HANDLE)_beginthreadex(NULL, 0, NetTickThread, 0, 0, &threadID);
	return 0L;
}
...
LRESULT WINAPI NetTickThread (LPVOID lp)
{
 ...
 //   CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE)NetSetupThread, 0, 0, &id);
	...
	HANDLE hThread;
	unsigned threadID;
	hThread = (HANDLE)_beginthreadex(NULL, 0, NetSetupThread, 0, 0, &threadID);
